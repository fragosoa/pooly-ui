    ...
    <div style={{ position: 'relative', display: 'inline-flex', flexShrink: 0 }}>
      ...
      {visible && (
        <div style={{
          ...
          background: 'var(--surface-dark-2)', color: 'white',
...
export default function TrendsTab({ reports, locale }) {
  ...
  if (!hasOpenTrends && !hasTypedData) {
    return (
      <div className="reports-empty">
        <div style={{
          width: '4rem', height: '4rem', background: 'var(--primary-light)', color: 'var(--primary)',
  ...
  const trendColor = (delta) => {
    ...
    if (delta > 0.05) return 'var(--success)';
    if (delta < -0.05) return 'var(--danger)';
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={sentimentByTs} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
                ...
                <Line
                  ...
                  activeDot={{ r: 7, stroke: 'var(--info)', strokeWidth: 2, fill: 'white' }}
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
  ...
  return (
    <div className="card" style={{ padding: '1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', padding: '0.15rem 0.5rem', background: 'var(--primary-light)', color: 'var(--primary)', borderRadius: '999px', fontWeight: '600' }}>
          <Icon name="list-checks" size={11} /> {isES ? 'Opción múltiple' : 'Multiple choice'}
        </span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
          ...
          <tbody>
            {allOptions.map(option => {
              ...
              const deltaColor = delta === null ? 'var(--text-muted)' : delta > 1 ? 'var(--success)' : delta < -1 ? 'var(--danger)' : 'var(--text-secondary)';
              return (
...
function NumericTrend({ question, runs, hasMultipleRuns, isES, locale }) {
  ...
  const meanDelta = latestRun?.stats?.mean != null && prevRun?.stats?.mean != null
    ...
    : null;
  const deltaColor = meanDelta === null ? 'var(--text-muted)' : meanDelta > 0 ? 'var(--success)' : meanDelta < 0 ? 'var(--danger)' : 'var(--text-secondary)';
  ...
  return (
    <div className="card" style={{ padding: '1.25rem', overflow: 'visible' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', padding: '0.15rem 0.5rem', background: 'var(--bg-secondary)', color: 'var(--text-secondary)', borderRadius: '999px', fontWeight: '600', border: '1px solid var(--border)' }}>
          <Icon name="hash" size={11} /> {isES ? 'Numérica' : 'Numeric'}
        </span>
        <h5 style={{ margin: 0, fontSize: '0.9rem', fontWeight: '600', color: 'var(--text-primary)' }}>
          {question.question_text}
        </h5>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={chartData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
            ...
            <Tooltip
              ...
              wrapperStyle={{ zIndex: 10 }}
            />
            <Line type="monotone" dataKey="mean" stroke="var(--info)" strokeWidth={2} dot={{ fill: 'var(--info)', r: 4 }} activeDot={{ r: 6 }} />
            <Line type="monotone" dataKey="median" stroke="var(--info-soft)" strokeWidth={1.5} strokeDasharray="4 2" dot={false} />
          </LineChart>
        </ResponsiveContainer>
      ) : (
...
function DateTrend({ question, runs, hasMultipleRuns, isES, locale }) {
  ...
  return (
    <div className="card" style={{ padding: '1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', padding: '0.15rem 0.5rem', background: 'var(--bg-secondary)', color: 'var(--text-secondary)', borderRadius: '999px', fontWeight: '600', border: '1px solid var(--border)' }}>
          <Icon name="calendar" size={11} /> {isES ? 'Fecha' : 'Date'}
        </span>
        <h5 style={{ margin: 0, fontSize: '0.9rem', fontWeight: '600', color: 'var(--text-primary)' }}>
          {question.question_text}
        </h5>
